...
import { Database, ShieldCheck, RefreshCw, ArrowLeft } from "lucide-react";
import { Dialog, DialogTrigger, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog";
import { useState } from "react";

export default function BaseDatosPage() {
    const [tipoBD, setTipoBD] = useState<string>("");
    ...
    const tipos = [
        {
            key: "pequena",
            label: "Base de datos pequeña",
            desc: "Hasta 10 tablas, ideal para webs sencillas, blogs o pequeños comercios.",
            precio: 80,
        },
        {
            key: "grande",
            label: "Base de datos grande",
            desc: "Más de 10 tablas, relaciones complejas, para ERPs, CRMs, tiendas grandes, etc.",
            precio: 180,
        },
    ];
    const preciosExtras = { "Copias de seguridad automáticas": 20, "Migración de datos": 30, "Optimización de consultas": 40 };
    const calcularPresupuesto = () => {
        const base = tipos.find(t => t.key === tipoBD)?.precio || 0;
        let total = base;
        ...
        <div className="min-h-screen relative py-20 bg-[url('/code-blue.jpg')] bg-cover bg-center">
            ...
            <div className="container mx-auto px-4 lg:px-6 max-w-3xl relative z-10 bg-gray-900/80 border border-blue-700 rounded-2xl shadow-2xl backdrop-blur-md p-8">
                ...
                <p className="text-lg text-gray-300 mb-8 text-center">
                    Diseño y desarrollo de bases de datos a medida, optimizadas para tu proyecto. Desde soluciones sencillas hasta sistemas complejos, con asesoría y soporte incluidos.
                </p>
                <div className="grid md:grid-cols-2 gap-6 mb-10">
                    <div className="flex flex-col items-center bg-gray-800 rounded-xl p-6 border border-gray-700">
                        ...
                        <span className="font-semibold text-white mb-1">Base de datos pequeña</span>
                        <span className="text-gray-400 text-sm text-center">Hasta 10 tablas, ideal para webs sencillas, blogs o pequeños comercios.</span>
                        <span className="text-blue-400 font-bold mt-2">80€</span>
                    </div>
                    <div className="flex flex-col items-center bg-gray-800 rounded-xl p-6 border border-gray-700">
                        <ShieldCheck className="w-8 h-8 text-green-400 mb-2" />
                        <span className="font-semibold text-white mb-1">Base de datos grande</span>
                        <span className="text-gray-400 text-sm text-center">Más de 10 tablas, relaciones complejas, para ERPs, CRMs, tiendas grandes, etc.</span>
                        <span className="text-green-400 font-bold mt-2">180€</span>
                    </div>
                </div>
                <div className="bg-blue-900/30 border border-blue-700 rounded-xl p-6 mb-8 text-center">
                    ...
                    <span className="block text-blue-200 mb-2">Desde <span className="font-bold text-2xl text-blue-400">80€</span> (precio orientativo, depende de la complejidad y extras).</span>
                        ...
                        <DialogContent className={`${modalBg} ${borderColor} ${shadow}`} style={{ color: '#222' }}>
                            ...
                            <form className="space-y-4" onSubmit={async (e) => {
                                ...
                                const res = await fetch("/api/presupuesto", {
                                    ...
                                    body: JSON.stringify({ tipoProyecto: tipoBD, extras, descripcion, nombre, email, presupuesto })
                                });
                                ...
                                <div className="mb-2 text-center">
                                    <span className="text-sm text-gray-500">¿Solo tienes una consulta rápida? <a href="https://wa.me/34657285571?text=Hola%2C%20tengo%20una%20consulta%20sobre%20tu%20servicio%20de%20bases%20de%20datos" target="_blank" rel="noopener noreferrer" className="text-green-600 underline hover:text-green-700">Háblame por WhatsApp</a></span>
                                </div>
                                <div>
                                    <label className={`block ${labelText} font-semibold mb-1`}>Tipo de base de datos</label>
                                    <div className="flex flex-col gap-2">
                                        {tipos.map((t) => (
                                            <label key={t.key} className="flex items-start gap-2 text-gray-700 cursor-pointer">
                                                <input type="radio" name="tipoBD" value={t.key} checked={tipoBD === t.key} onChange={() => setTipoBD(t.key)} className="mt-1" />
                                                <span>
