...
const AgusBot = memo(function AgusBot() {
    ...
    return (
        <>
            <button
                onClick={() => setOpen((o) => !o)}
                className="fixed bottom-6 right-6 z-50 bg-white hover:bg-gray-100 rounded-full shadow-lg w-16 h-16 flex items-center justify-center text-3xl border-4 border-white touch-target"
            ...
            {open && (
                <div className="fixed bottom-24 right-6 z-50 w-80 max-w-[90vw] bg-white rounded-2xl shadow-2xl border border-blue-200 flex flex-col overflow-hidden animate-fade-in no-scroll-horizontal">
                    <div className="flex items-center gap-2 p-4 bg-blue-600 text-white">
                        ...
                        <button onClick={() => setOpen(false)} className="ml-auto text-white text-xl hover:text-blue-200 touch-target">×</button>
                    </div>
                    <div className="flex-1 p-4 space-y-2 overflow-y-auto max-h-72 bg-blue-50 scrollbar-hide">
                        {messages.map((msg, i) => (
                            <div key={i} className={msg.from === "bot" ? "text-left" : "text-right"}>
                                <span className={msg.from === "bot" ? "inline-block bg-blue-100 text-blue-900 rounded-xl px-3 py-2 mb-1 break-words" : "inline-block bg-blue-500 text-white rounded-xl px-3 py-2 mb-1 break-words"}>
                                    {msg.text}
                                </span>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {msg.options.map((opt, j) => (
                                            <button
                                                ...
                                                className="bg-blue-200 hover:bg-blue-300 text-blue-900 rounded-lg px-3 py-1 text-xs font-semibold shadow-sm border border-blue-300 touch-target"
                                            >{opt}</button>
                                        ))}
                                    </div>
                                )}
                ...
                <div className="container mx-auto px-4 lg:px-6 h-16 flex items-center justify-between">
                    ...
                    {false && (
                        <nav className="hidden md:flex items-center space-x-8">
                            ...
                            <Link href="#sobre-mi" className="text-gray-300 hover:text-white transition-colors touch-target">
                                Sobre Mí
                            </Link>
                            <Link href="#contacto" className="text-gray-300 hover:text-white transition-colors touch-target">
                                Contacto
                            </Link>
                        </nav>
                    )}
                    <div className="hidden items-center space-x-2 lg:space-x-4">
                        ...
                        <a href="https://wa.me/34657285571?text=Buenos%20dias%2C%20he%20visto%20tu%20web%20y%20estoy%20interesado%20en%20alguno%20de%20tus%20servicio%2C%20%C2%BFcuando%20podemos%20concretar%20una%20cita%3F" target="_blank" rel="noopener noreferrer">
                            <Button className="bg-green-500 hover:bg-green-600 text-white flex items-center gap-2 touch-target">
